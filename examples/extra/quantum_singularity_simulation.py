...
def simulate_quantum_singularity(initial_state, epsilon, t_range):
    ...
    t_values = dbz.linspace(t_range[0], t_range[1], 400)
...
def run_multiple_simulations():
    ...
    initial_states = [
        dbz.array([1, 0]) / dbz.sqrt(2),  # |+⟩ state
        dbz.array([0, 1]) / dbz.sqrt(2),  # |-⟩ state
        dbz.array([1, 1]) / dbz.sqrt(2),  # |+⟩ + |-⟩ state
    ]
...
def plot_results(results):
    for i, res in enumerate(results):
        ...
        plt.subplot(2, 1, 1)
        state_evolutions = dbz.array([state.data.flatten() for state in evolved_states])
        for j in range(2):
            plt.plot(t_values, dbz.abs(state_evolutions[:, j])**2, label=f'|ψ{j}|²')
